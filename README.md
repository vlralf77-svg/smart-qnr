...
> 본 저장소는 **프론트엔드(문진 에디터/렌더러) 우선** 단계입니다. 문서 파싱·LLM 변환·
> Oracle 저장 등 백엔드(Java/Spring Boot)는 후속 Phase에서 연결합니다.
...
| QNR002 문서 변환 | `/#/upload` | 변환 결과 JSON 임포트 / 빈 문진 시작 (LLM 변환은 백엔드 Phase) |
| QNR003 문진 에디터 | `/#/editor/:id` | 섹션/문항 추가·수정·삭제·드래그 정렬, 선택지·조건부 로직, 실시간 미리보기, 발행 |
| QNR004 응답 화면 | `/#/respond/:id` | 발행 문진 응답·제출 (웹/모바일 공용) |

## 핵심 계약(스키마)

프론트/백엔드/LLM 이 공유하는 단일 문진 스키마는 [`src/types/schema.ts`](src/types/schema.ts)
에 정의되어 있다. 임의 입력(LLM 변환 결과·JSON 임포트)은
...
- **현재(프론트)**: 에디터/렌더러/목록, 스키마 계약·검증, `.exe` 패키징 골격
- **백엔드 Phase**: 문서 파서(PDF/DOCX→HWP), Claude LLM 변환, Oracle 저장/버전, REST API
