...
| 실행 구조             | 독립 실행형(Self-contained)     | embedded Tomcat(`starter-web`) + 실행가능 jar(`spring-boot-maven-plugin`) → `java -jar`, 외부 WAS 불필요 | ✅   |
...
| 요구사항  | 요구값          | 현재                                                                            | 판정 |
| --------- | --------------- | ------------------------------------------------------------------------------- | ---- |
| 동시 접속 | 100명 정상 작동 | Tomcat `max-threads=200` + HikariCP `pool=20` + nginx 정적 서빙(경량 CRUD 충분) | ✅   |
| 검증 수단 | 부하 테스트     | `scripts/loadtest.js`(k6, 100 VU, 오류율<1%·p95<800ms) + `docs/PERFORMANCE.md`  | ✅   |

- 상세: [`docs/PERFORMANCE.md`](./PERFORMANCE.md) (구간별 용량·설정·확장 경로·부하 테스트 방법)
- 최종 "정상 작동" 판정은 실 배포 스택에서 부하 테스트 실행 결과로 확정.
...
| CI/CD 파이프라인  | 게이트형(`ci.yml`: 품질·백엔드 통과 → 이미지 빌드/푸시) + 데스크톱 자동배포 + Jenkins 정의 · `docs/CICD.md` | ✅   |
