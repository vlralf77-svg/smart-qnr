...
// 딩벳 동그라미(❶-❿, ➀-➓), ⓪, 체크표시 등
const CHECKBOX_RE = /[□○◯▢☐■◻◼⬜⬛✓✔☑◇◆①-⑳⑴-⒇⓪❶-❿➀-➓]/gu;
const MARKER_SPLIT_RE = /[\s]{2,}|[□○◯▢☐■◻◼⬜⬛✓✔☑◇◆①-⑳⑴-⒇⓪❶-❿➀-➓]/u;
...
export async function pdfToOverlayForm(
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const detected = detectFieldsInPage(items, viewport.transform, RENDER_SCALE, cw, ch, i - 1);
  ...
  const schema: FormSchema = {
    ...
    description:
      '원본 PDF 위에 입력필드를 얹은 문진입니다. 자동 배치된 필드 위치·유형을 확인·조정하세요.',
    version: 1,
    status: 'draft',
